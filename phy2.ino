...
#define TURN_DELAY_CYCLES 1  // Number of cycles to wait before initiating a turn
...
void robotAutoMotorMove(struct Robot * robot, int front_centre_sensor, int left_sensor, int right_sensor) {
    ...
    if (front_centre_sensor != 0) {
        ...
        setSpeed(robot, 0);  // Ensure the robot doesn't move forward
    ...
    // If the front center sensor doesn't detect a wall, continue moving forward
        ...
        if (left_sensor > 3 && left_sensor <= 5 && !was_adjusted) {
            ...
            setSpeed(robot, ADJUSTMENT_SPEED);  // Slow down when adjusting
            was_adjusted = 1;
        } 
            ...
            setSpeed(robot, CONSTANT_SPEED);  // Ensure to revert back to normal speed
        }
    // Output the robot's angle and speed after each iteration
    printf("Robot's angle: %d | Robot's speed: %d\n", robot->angle, robot->currentSpeed);
    }
}

#define PULSE_STOP 1500
#define PULSE_MAX_FORWARD 1700
#define PULSE_MAX_BACKWARD 1300
...
void controlMotorsBasedOnNavigation(struct Robot* robot) {
    ...
    int leftServoPulse = PULSE_STOP;
    int rightServoPulse = PULSE_STOP;

    // Map the robot's current speed to the appropriate pulse widths
    leftServoPulse = map(robot->currentSpeed, -1, 1, PULSE_STOP, PULSE_MAX_FORWARD);
    rightServoPulse = map(robot->currentSpeed, -1, 1, PULSE_MAX_FORWARD, PULSE_STOP);

    Serial.print("Left servo pulse: ");
    Serial.println(leftServoPulse);
    Serial.print("Right servo pulse: ");
    Serial.println(rightServoPulse);
    ...
    if(robot->currentSpeed > 0) {
        ...
        if(robot->angle > 0) {
            ...
            rightServoPulse = min(rightServoPulse + turnAdjustment, PULSE_MAX_FORWARD);
        } else if(robot->angle < 0) {
            // Left Turn: Slow down the left wheel
            leftServoPulse = max(leftServoPulse - turnAdjustment, PULSE_STOP);
        }
    }

    // Write the determined pulse widths to the servos
    servoLeft.writeMicroseconds(leftServoPulse);
    servoRight.writeMicroseconds(rightServoPulse);
}
